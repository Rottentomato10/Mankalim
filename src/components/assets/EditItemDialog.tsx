...
interface EditItemDialogProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (data: { name?: string; isLiquid?: boolean; currency?: string; notes?: string | null }) => void
  onDelete: () => void
  type: ItemType
  item: {
    name: string
...
const typeLabels: Record<ItemType, string> = {
  assetClass: 'עריכת קטגוריה',
  instrument: 'עריכת מכשיר',
  provider: 'עריכת ספק',
  asset: 'עריכת נכס',
}
...
export function EditItemDialog({ isOpen, onClose, onSubmit, onDelete, type, item, isLoading, childCount = 0 }: EditItemDialogProps) {
      ...
      <GlassCard className="relative w-full max-w-md animate-in fade-in zoom-in-95 duration-200 !p-0">
        {showDeleteConfirm ? (
          <div className="p-6 space-y-4">
            <h2 className="text-lg font-semibold text-error">אישור מחיקה</h2>
            <p className="text-white/70">
              האם למחוק את &quot;{item.name}&quot;?
              {childCount > 0 && (
                <span className="block mt-2 text-error/80">
                  שים לב: פעולה זו תמחק גם {childCount} פריטים משויכים
                </span>
              )}
            </p>
            <div className="flex gap-3 pt-2">
              ...
              <button
                onClick={onDelete}
                disabled={isLoading}
                className="flex-1 py-2.5 px-4 rounded-xl bg-error hover:bg-error/90 text-white font-medium transition-colors disabled:opacity-50"
              >
                {isLoading ? 'מוחק...' : 'מחק'}
              </button>
            </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="p-6 space-y-4">
              ...
              {type === 'asset' && (
                <>
                  ...
                  <div>
                    <label className="text-white/70 text-sm block mb-2">הערות</label>
                    <textarea
                      value={notes}
            ...
            <div className="flex gap-3 p-4 border-t border-glass-border">
              <button
                ...
                disabled={isLoading}
                className="py-2.5 px-4 rounded-xl bg-error/10 hover:bg-error/20 text-error font-medium transition-colors disabled:opacity-50"
              >
                מחק
              </button>
              ...
              <button
                ...
                onClick={onClose}
                disabled={isLoading}
                className="py-2.5 px-4 rounded-xl bg-white/10 hover:bg-white/15 text-white font-medium transition-colors disabled:opacity-50"
              >
                ביטול
              </button>
              <button
                ...
                className="py-2.5 px-4 rounded-xl bg-accent hover:bg-accent/90 text-background font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading ? 'שומר...' : 'שמור'}
              </button>
            </div>
